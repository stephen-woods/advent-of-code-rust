...
//! Unfortunately, the password seems to have been changed, so you can't get in. A document taped
//! to the wall helpfully explains:
//!
...
//!
//! So, if the dial were pointing at 5, a rotation of L10 would cause it to point at 95. After
...
fn part_a() -> i32 {
    ...
    for line in INPUT.lines() {
        ...
        let ticks = match &trimmed[0..1] {
            "L" => -trimmed[1..].parse::<i32>().unwrap(),
...
fn part_b() -> i32 {
    ...
    for line in INPUT.lines() {
        let trimmed = line.trim();
        let x = match &trimmed[0..1] {
            "L" => -1,
            "R" => 1,
            _ => panic!("Unexpected direction"),
        };

        let ticks = trimmed[1..].parse::<i32>().unwrap();
